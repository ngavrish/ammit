...
ts("Suite runs, passed against failed",
   "Every suite run of the branch loop as a pair of bars, passed beside failed, titled by branch, from the suite event the suite reports. Failed bars that do not shrink lap after lap are a loop healing nothing. A red suite with nothing failed is the runner exiting non-zero, not the tests; the table below says why.", "scenarios", [
    q("""SELECT at AS time, 'passed' AS metric, passed AS value, coalesce(nullif(branch,''),'(no branch)') AS label FROM (SELECT at, run, coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.verdict') AS verdict, json_extract(payload,'$.total') AS total, json_extract(payload,'$.passed') AS passed, json_extract(payload,'$.failed') AS failed, coalesce(json_extract(payload,'$.reason'),'') AS reason FROM events WHERE kind='suite' UNION ALL SELECT at, run, coalesce(json_extract(payload,'$.branch'),''), CASE WHEN instr(json_extract(payload,'$.text'),'SUITE GREEN')>0 THEN 'green' ELSE 'red' END, CAST(substr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')), instr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')),': ')+2) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'scenarios, ')+11) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'passed, ')+8) AS INTEGER), 'read from the phase text: before the suite event' FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase')='branchrun' AND run NOT IN (SELECT run FROM events WHERE kind='suite')) WHERE at*1000 BETWEEN $__from AND $__to UNION ALL SELECT at, 'failed', failed, coalesce(nullif(branch,''),'(no branch)') FROM (SELECT at, run, coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.verdict') AS verdict, json_extract(payload,'$.total') AS total, json_extract(payload,'$.passed') AS passed, json_extract(payload,'$.failed') AS failed, coalesce(json_extract(payload,'$.reason'),'') AS reason FROM events WHERE kind='suite' UNION ALL SELECT at, run, coalesce(json_extract(payload,'$.branch'),''), CASE WHEN instr(json_extract(payload,'$.text'),'SUITE GREEN')>0 THEN 'green' ELSE 'red' END, CAST(substr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')), instr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')),': ')+2) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'scenarios, ')+11) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'passed, ')+8) AS INTEGER), 'read from the phase text: before the suite event' FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase')='branchrun' AND run NOT IN (SELECT run FROM events WHERE kind='suite')) WHERE at*1000 BETWEEN $__from AND $__to ORDER BY 1""")], 0, points=True)

tbl("Heal loop by branch",
    "One row per branch: how many times its suite ran, how many heal laps the runner took and what the cap was, what the last lap decided (again, converged, gave_up), what the laps cost in minutes and dollars, the failures at the first suite against the last, and why the last suite was red in the suite's own words.",
    """SELECT coalesce(nullif(s.branch,''),'(no branch)') AS branch, count(*) AS suite_runs, coalesce(l.laps, h.laps, 0) AS heal_laps, l.cap AS cap, l.decision AS last_lap, coalesce(h.minutes,0) AS heal_minutes, coalesce(m.usd,0) AS heal_usd, min(CASE WHEN s.n=1 THEN s.failed END) AS failed_first, min(CASE WHEN s.rn=1 THEN s.failed END) AS failed_last, min(CASE WHEN s.rn=1 THEN s.verdict END) AS last_suite, min(CASE WHEN s.rn=1 THEN s.reason END) AS why FROM (SELECT *, row_number() OVER (PARTITION BY branch ORDER BY at) AS n, row_number() OVER (PARTITION BY branch ORDER BY at DESC) AS rn FROM (SELECT at, run, coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.verdict') AS verdict, json_extract(payload,'$.total') AS total, json_extract(payload,'$.passed') AS passed, json_extract(payload,'$.failed') AS failed, coalesce(json_extract(payload,'$.reason'),'') AS reason FROM events WHERE kind='suite' UNION ALL SELECT at, run, coalesce(json_extract(payload,'$.branch'),''), CASE WHEN instr(json_extract(payload,'$.text'),'SUITE GREEN')>0 THEN 'green' ELSE 'red' END, CAST(substr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')), instr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')),': ')+2) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'scenarios, ')+11) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'passed, ')+8) AS INTEGER), 'read from the phase text: before the suite event' FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase')='branchrun' AND run NOT IN (SELECT run FROM events WHERE kind='suite')) WHERE at*1000 BETWEEN $__from AND $__to) s LEFT JOIN (SELECT branch, max(lap) AS laps, max(cap) AS cap, max(CASE WHEN rn=1 THEN decision END) AS decision FROM (SELECT coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.lap') AS lap, json_extract(payload,'$.cap') AS cap, json_extract(payload,'$.decision') AS decision, row_number() OVER (PARTITION BY coalesce(json_extract(payload,'$.branch'),'') ORDER BY at DESC) AS rn FROM events WHERE kind='heal_lap' AND at*1000 BETWEEN $__from AND $__to) GROUP BY branch) l ON l.branch=s.branch LEFT JOIN (SELECT coalesce(json_extract(payload,'$.branch'),'') AS branch, count(*) AS laps, round(sum(json_extract(payload,'$.seconds'))/60.0,1) AS minutes FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase') LIKE '%heal%' AND at*1000 BETWEEN $__from AND $__to GROUP BY 1) h ON h.branch=s.branch LEFT JOIN (SELECT coalesce(json_extract(payload,'$.branch'),'') AS branch, round(sum(json_extract(payload,'$.usd')),2) AS usd FROM events WHERE kind='spend' AND json_extract(payload,'$.phase') LIKE '%heal%' AND at*1000 BETWEEN $__from AND $__to GROUP BY 1) m ON m.branch=s.branch GROUP BY s.branch ORDER BY heal_laps DESC, suite_runs DESC""", 0, ())
...
ABOUT = {
    "Suite runs, passed against failed":
        "Every suite run of the branch loop as a pair of bars, passed beside failed, titled by branch, from the suite event the suite reports. Failed bars that do not shrink lap after lap are a loop healing nothing. A red suite with nothing failed is the runner exiting non-zero, not the tests; the table below says why.",
    "Heal loop by branch":
        "One row per branch: how many times its suite ran, how many heal laps the runner took and what the cap was, what the last lap decided (again, converged, gave_up), what the laps cost in minutes and dollars, the failures at the first suite against the last, and why the last suite was red in the suite's own words.",
...
heal = {
    ...
    "panels": [
        ...
        },
        {
            ...
            "about": "Every run in the order it started: heal laps over all its branches. The number the memory is judged by: it has to fall as lessons accumulate, or nothing is being learned. From the runner's own heal_lap events; runs from before the event are counted from heal phases.",
            "unit": "laps",
            "height": 8,
            "queries": [
                "SELECT r.started AS time, 'heal laps' AS metric, coalesce(l.total, h.total, 0) AS value, r.name AS label FROM runs r LEFT JOIN (SELECT run, sum(laps) AS total, max(laps) AS most FROM (SELECT run, coalesce(json_extract(payload,'$.branch'),'') AS b, max(json_extract(payload,'$.lap')) AS laps FROM events WHERE kind='heal_lap' GROUP BY 1,2) GROUP BY run) l ON l.run=r.run LEFT JOIN (SELECT run, sum(n) AS total, max(n) AS most FROM (SELECT run, count(*) AS n FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase') LIKE '%heal%' GROUP BY run, coalesce(json_extract(payload,'$.branch'),'')) GROUP BY run) h ON h.run=r.run ORDER BY 1"
            ]
        },
        {
            "kind": "bars",
            "title": "Heal laps on one branch, against the cap",
            "about": "Per run, the most heal laps any one branch took, against the cap the runner gives up at - limits.heal_laps_per_branch, read from ammit at every decision. A bar at the line is a branch that spent its whole budget; over it is a run from before the cap was a limit.",
            "unit": "laps",
            "height": 8,
            "queries": [
                "SELECT r.started AS time, 'most on one branch' AS metric, coalesce(l.most, h.most, 0) AS value, r.name AS label FROM runs r LEFT JOIN (SELECT run, sum(laps) AS total, max(laps) AS most FROM (SELECT run, coalesce(json_extract(payload,'$.branch'),'') AS b, max(json_extract(payload,'$.lap')) AS laps FROM events WHERE kind='heal_lap' GROUP BY 1,2) GROUP BY run) l ON l.run=r.run LEFT JOIN (SELECT run, sum(n) AS total, max(n) AS most FROM (SELECT run, count(*) AS n FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase') LIKE '%heal%' GROUP BY run, coalesce(json_extract(payload,'$.branch'),'')) GROUP BY run) h ON h.run=r.run ORDER BY 1",
                "SELECT at AS time, name AS metric, value FROM limits WHERE name = 'limits.heal_laps_per_branch' AND at*1000 <= $__to AND (at*1000 >= $__from OR at = (SELECT max(at) FROM limits WHERE name = 'limits.heal_laps_per_branch' AND at*1000 < $__from)) ORDER BY at"
            ]
        },
        {
            "kind": "bars",
            "title": "Heal laps after a suite with nothing failed",
            "about": "Per run: heal laps that followed a suite reporting zero failed scenarios. The suite was red for another reason - the runner exiting non-zero, no scenario mapped - and the healer was sent to fix tests that passed. Run c306ccfe did this four times on one branch. From the suite events; older runs from the phase text.",
            "unit": "laps",
            "height": 8,
            "queries": [
                "SELECT r.started AS time, 'laps on green' AS metric, coalesce(g.n,0) AS value, r.name AS label FROM runs r LEFT JOIN (SELECT run, count(*) AS n FROM (SELECT e.run, (SELECT s.failed FROM (SELECT at, run, coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.verdict') AS verdict, json_extract(payload,'$.total') AS total, json_extract(payload,'$.passed') AS passed, json_extract(payload,'$.failed') AS failed, coalesce(json_extract(payload,'$.reason'),'') AS reason FROM events WHERE kind='suite' UNION ALL SELECT at, run, coalesce(json_extract(payload,'$.branch'),''), CASE WHEN instr(json_extract(payload,'$.text'),'SUITE GREEN')>0 THEN 'green' ELSE 'red' END, CAST(substr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')), instr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')),': ')+2) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'scenarios, ')+11) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'passed, ')+8) AS INTEGER), 'read from the phase text: before the suite event' FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase')='branchrun' AND run NOT IN (SELECT run FROM events WHERE kind='suite')) s WHERE s.run=e.run AND s.branch=coalesce(json_extract(e.payload,'$.branch'),'') AND s.at < e.at ORDER BY s.at DESC LIMIT 1) AS failed_before FROM events e WHERE e.kind='phase_start' AND json_extract(payload,'$.phase') LIKE '%heal%') WHERE failed_before = 0 GROUP BY run) g ON g.run=r.run ORDER BY 1"
            ]
        },
        {
            "kind": "bars",
            "title": "Suite failures, first against last",
            "about": "Per run, the failing scenarios summed over its branches at the first suite run and at the last, from the suite events. The distance between the two bars is what the heal loop bought; a last bar as tall as the first is a loop that ran and repaired nothing.",
            "unit": "scenarios",
            "height": 8,
            "queries": [
                "SELECT r.started AS time, 'first suite' AS metric, coalesce(sum(CASE WHEN s.n=1 THEN s.failed END),0) AS value, r.name AS label FROM runs r LEFT JOIN (SELECT *, row_number() OVER (PARTITION BY run, branch ORDER BY at) AS n, row_number() OVER (PARTITION BY run, branch ORDER BY at DESC) AS rn FROM (SELECT at, run, coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.verdict') AS verdict, json_extract(payload,'$.total') AS total, json_extract(payload,'$.passed') AS passed, json_extract(payload,'$.failed') AS failed, coalesce(json_extract(payload,'$.reason'),'') AS reason FROM events WHERE kind='suite' UNION ALL SELECT at, run, coalesce(json_extract(payload,'$.branch'),''), CASE WHEN instr(json_extract(payload,'$.text'),'SUITE GREEN')>0 THEN 'green' ELSE 'red' END, CAST(substr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')), instr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')),': ')+2) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'scenarios, ')+11) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'passed, ')+8) AS INTEGER), 'read from the phase text: before the suite event' FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase')='branchrun' AND run NOT IN (SELECT run FROM events WHERE kind='suite'))) s ON s.run=r.run GROUP BY r.run UNION ALL SELECT r.started, 'last suite', coalesce(sum(CASE WHEN s.rn=1 THEN s.failed END),0), r.name FROM runs r LEFT JOIN (SELECT *, row_number() OVER (PARTITION BY run, branch ORDER BY at) AS n, row_number() OVER (PARTITION BY run, branch ORDER BY at DESC) AS rn FROM (SELECT at, run, coalesce(json_extract(payload,'$.branch'),'') AS branch, json_extract(payload,'$.verdict') AS verdict, json_extract(payload,'$.total') AS total, json_extract(payload,'$.passed') AS passed, json_extract(payload,'$.failed') AS failed, coalesce(json_extract(payload,'$.reason'),'') AS reason FROM events WHERE kind='suite' UNION ALL SELECT at, run, coalesce(json_extract(payload,'$.branch'),''), CASE WHEN instr(json_extract(payload,'$.text'),'SUITE GREEN')>0 THEN 'green' ELSE 'red' END, CAST(substr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')), instr(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'SUITE ')),': ')+2) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'scenarios, ')+11) AS INTEGER), CAST(substr(json_extract(payload,'$.text'), instr(json_extract(payload,'$.text'),'passed, ')+8) AS INTEGER), 'read from the phase text: before the suite event' FROM events WHERE kind='phase_end' AND json_extract(payload,'$.phase')='branchrun' AND run NOT IN (SELECT run FROM events WHERE kind='suite'))) s ON s.run=r.run GROUP BY r.run ORDER BY 1"
            ]
        },
        {
            "kind": "bars",
